...
class Rocket(tile: RocketTile)(implicit p: Parameters) extends CoreModule()(p)
  ...
  // indicates whether there is a new, valid instruction available at the decode stage input
  val dec_valid = ibuf.io.inst(0).valid && !take_pc
  ...
  val (id_raddr2_illegal, raw_id_raddr2) = decodeReg(id_expanded_inst(0).rs2)
  val (id_raddr1_illegal, raw_id_raddr1) = decodeReg(id_expanded_inst(0).rs1)
  val (id_waddr_illegal,  raw_id_waddr)  = decodeReg(id_expanded_inst(0).rd)

  val id_raddr2 = Wire(UInt(5.W))
  val id_raddr1 = Wire(UInt(5.W))
  val id_waddr  = Wire(UInt(5.W))

  id_raddr2 := raw_id_raddr2
  id_raddr1 := raw_id_raddr1
  id_waddr  := raw_id_waddr
  ...
  // Detect lui and addi by opcode
  val is_lui  = (id_inst(0)(6,0) === "b0110111".U)
  val is_addi = (id_inst(0)(6,0) === "b0010011".U) && ((id_inst(0)(14,12) === "b000".U))

  // extract rd feld and immediate field
  val id_rd = id_inst(0)(11,7)

  // for not I-type instruction, id_imm value is invalid
  val id_imm_i = ImmGen(IMM_I, id_inst(0))

  val lui_imm = WireDefault(0.S(32.W))
  when (is_lui) {
    lui_imm := Cat(id_inst(0)(31,12), 0.U(12.W)).asSInt
  }.otherwise {
    lui_imm := 0.S  // Ensure lui_imm is 0 for non-LUI instructions
  }
  val id_imm = Wire(SInt(32.W))
  id_imm := Mux(is_lui, lui_imm, id_imm_i)


  val prev_was_lui = RegInit(false.B)
  val prev_lui_rd  = Reg(UInt(5.W))
  val prev_lui_imm = Reg(SInt(32.W))

  val waiting_for_addi = RegInit(false.B)

  when (id_ctrl.legal && !id_xcpt && is_lui && !waiting_for_addi) {
    prev_was_lui := true.B
    prev_lui_rd  := id_rd
    prev_lui_imm := lui_imm
  }.elsewhen(!waiting_for_addi) {
    prev_was_lui := false.B
  }

  val stall_decode = waiting_for_addi

  
  when (id_ctrl.legal && !id_xcpt && is_lui && !waiting_for_addi) {
    waiting_for_addi := true.B
  }.elsewhen(waiting_for_addi && dec_valid) {
    waiting_for_addi := false.B
  }
  

  val addi_rs1 = id_inst(0)(19,15)
  val fuse_possible = prev_was_lui && is_addi && (addi_rs1 === prev_lui_rd) && !id_xcpt && id_ctrl.legal

  val fused_imm = (prev_lui_imm + id_imm)


  val fused_imm_final = Wire(SInt(32.W))
  fused_imm_final := id_imm  // By default, no change to imm

  val fused_rd = Wire(UInt(5.W))
  fused_rd := id_waddr       

  val fused_rs1 = Wire(UInt(5.W))
  fused_rs1 := id_raddr1     

  val fused_wxd = Wire(Bool())
  fused_wxd := id_ctrl.wxd

  val fused_rxs1 = Wire(Bool())
  fused_rxs1 := id_ctrl.rxs1

  val fused_rxs2 = Wire(Bool())
  fused_rxs2 := id_ctrl.rxs2

  val fused_alu_fn = Wire(id_ctrl.alu_fn.cloneType)
  fused_alu_fn := id_ctrl.alu_fn

  when (fuse_possible) {
    fused_imm_final := fused_imm
    fused_rs1 := 0.U
    fused_rd := prev_lui_rd

    fused_wxd := true.B
    fused_rxs1 := false.B
    fused_rxs2 := false.B

    fused_alu_fn := ALU.FN_ADD
  }
  ...
  ex_reg_valid := !ctrl_killd && dec_valid && !stall_decode
  ...
  when (!ctrl_killd && !stall_decode) {
    ...
    when (id_ctrl.mem_cmd.isOneOf(M_SFENCE, M_HFENCEV, M_HFENCEG, M_FLUSH_ALL)) {
      ...
      ex_reg_mem_size := Cat(id_raddr2 =/= 0.U, fused_rs1 =/= 0.U)
    }
  ...
  */
  val hazard_targets = Seq((fused_rxs1 && fused_rs1 =/= 0.U, fused_rs1),
                           (fused_rxs2 && id_raddr2 =/= 0.U, id_raddr2),
                           (fused_wxd  && fused_rd  =/= 0.U, fused_rd))
  /*
  ...
  */
  val fp_hazard_targets = Seq((io.fpu.dec.ren1, fused_rs1),
                              (io.fpu.dec.ren2, id_raddr2),
                              (io.fpu.dec.ren3, id_raddr3),
                              (io.fpu.dec.wen, fused_rd))
